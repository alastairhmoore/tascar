...
class rec_vbapn_t : public TASCAR::receivermod_base_speaker_t {
public:
  ...
  rec_vbapn_t(tsccfg::node_t xmlsrc);
  virtual ~rec_vbapn_t(){};
...
rec_vbapn_t::data_t::data_t(uint32_t channels)
{
...
rec_vbapn_t::data_t::~data_t()
{
  delete[] wp;
  delete[] dwp;
}

rec_vbapn_t::rec_vbapn_t(tsccfg::node_t xmlsrc)
...
*/
void rec_vbapn_t::add_pointsource(const TASCAR::pos_t& prel, double,
                                  const TASCAR::wave_t& chunk,
                                  std::vector<TASCAR::wave_t>& output,
...
rec_vbapn_t::create_state_data(double, uint32_t) const
{
  return new data_t(spkpos.size());
}

REGISTER_RECEIVERMOD(rec_vbapn_t);
